...
return {
  -- File explorer
  {
    ...
    keys = {
      ...
      { "<leader>fE", "<cmd>Neotree toggle dir=getcwd()<cr>", desc = "Explorer NeoTree (cwd)" },
  ...
  -- Session persistence (auto-save/restore per project)
  {
    ...
    config = function(_, opts)
      ...
      -- Auto-save session on exit (if working in a project)
      vim.api.nvim_create_autocmd("VimLeavePre", {
        callback = function()
          -- Only save if in a git repo or has a root marker
          if vim.fn.isdirectory(".git") == 1 or
             vim.fn.filereadable("package.json") == 1 or
             vim.fn.filereadable("Cargo.toml") == 1 or
             vim.fn.filereadable("go.mod") == 1 then
